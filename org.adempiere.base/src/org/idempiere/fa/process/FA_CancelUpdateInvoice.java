...
public class FA_CancelUpdateInvoice extends SvrProcess {
	...
	protected String doIt() throws Exception {
		...
		if (C_Invoice_ID > 0) {
			...
			sql.append("SELECT cil.C_Invoice_ID, cil.C_InvoiceLine_ID,cil.M_InOut_ID,cil.M_InOutLine_ID");
				...
				while (rs.next()) {
					...
					if (C_Charge_ID > 0 && M_Product_ID == 0) {
						if (!inv.get_ValueAsBoolean("IsTrackAsAsset")){		
							inv.set_ValueOfColumn("IsTrackAsAsset", false);
							inv.saveEx();
						}
						
						invLine.set_ValueOfColumn("IsTrackAsAsset", false);
						invLine.saveEx();
...
public String updatePO(int M_InOutLine_ID){
	...
	StringBuilder sqlUpdate = new StringBuilder();
	sqlUpdate.append("UPDATE C_Order SET isTrackAsAsset = 'N' ");
	sqlUpdate.append(" WHERE C_Order_ID = ?");
	sqlUpdate.append(" AND isTrackAsAsset = 'Y' ");
	
	StringBuilder sqlUpdateLine = new StringBuilder();
	sqlUpdateLine.append("UPDATE C_OrderLine SET isTrackAsAsset = 'N' ");
